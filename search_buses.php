...
if ($date !== '') {
    $stmt = $conn->prepare("SELECT * FROM buses WHERE source LIKE ? AND destination LIKE ? AND travel_date = ?");
    $stmt->bind_param("sss", $likeFrom, $likeTo, $date);
} else {
    $stmt = $conn->prepare("SELECT * FROM buses WHERE source LIKE ? AND destination LIKE ?");
